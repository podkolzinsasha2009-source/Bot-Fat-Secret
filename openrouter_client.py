...
⚠️ ПРАВИЛО ПЕРЕСЧЁТА ПОРЦИЙ — ОБЯЗАТЕЛЬНО:
Все базы данных дают КБЖУ на 100г. Пересчитай под реальный вес по формуле:
  значение_на_100г × (weight ÷ 100)
...
Значения КБЖУ в итоговом JSON не могут превышать вес порции в граммах.

⚠️ ЖЁСТКОЕ ПРАВИЛО: log_food vs edit_food
  log_food  → пользователь называет продукт и вес. ВСЕГДА новая отдельная строка,
              даже если этот продукт уже был сегодня (повторная порция = новая запись!).
  edit_food → ТОЛЬКО при явных словах-маркерах изменения: "измени", "исправь",
              "ошибка", "поменяй", "не Xг а Yг", "вместо X — Y".
              Нет таких слов → всегда log_food, не edit_food.
...
async def ask_gemini(user_text: str, context_info: str) -> dict:
    ...
    # Llama, OpenAI, Gemini — поддерживают json_object, включаем для надёжности.
