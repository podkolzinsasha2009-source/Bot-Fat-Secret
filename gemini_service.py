...
─── НАЗВАНИЯ ПРОДУКТОВ ─────────────────────────────────────────────
Сохраняй название ТОЧНО как сказал пользователь — с брендом, жирностью, сортом.
ЖИРНОСТЬ/ПРОЦЕНТ — ОБЯЗАТЕЛЬНО сохранять если упомянута:
  «молоко лебедянское 3.2» → name: «молоко лебедянское 3.2»
  «творог 5%» → name: «творог 5%»
  «кефир 2.5» → name: «кефир 2.5»
  «молоко» (без процента) → name: «молоко»
  «сахар» → name: «сахар»  ← именно «сахар», без добавок
Процент жирности важнее бренда для точного подбора КБЖУ.
"""
